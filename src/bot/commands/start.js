...
async function startHandler(ctx) {
  try {
    ...
    logger.error(`[StartHandler] Critical error:`, err);
    
    const maintenanceMsg = `🛠️ <b>Nossos servidores estão passando por uma rápida manutenção/atualização.</b>\n\n` +
                           `Estamos calibrando os sistemas para garantir a melhor performance de execução. Volte em alguns minutos!\n\n` +
                           `<i>Obrigado pela paciência, Comandante.</i>`;
                           
    if (ctx.callbackQuery) {
      return ctx.editMessageText(maintenanceMsg, { parse_mode: 'HTML' }).catch(() => {});
    }
    return ctx.replyWithHTML(maintenanceMsg).catch(() => {});
  }
}
